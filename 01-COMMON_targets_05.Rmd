# GO and KEGG: common targets 05

## Load the data

```{r}
data_MAPP <- read.table("../data/inclusion_fractions_FG_COMMON_targets_05.tsv", 
                        header = T)
```
...
```{r}
if(nrow(KEGGresults@result %>% filter(p.adjust < 0.1)) > 0){
  DT::datatable(KEGGresults@result %>% filter(p.adjust < 0.1), 

...
```{r}
universe <- read.table("../data/inclusion_fractions_ALL_updated.tsv", 
...
```{r}
ENTREZ_universe <- read.table("../data/inclusion_fractions_ALL_updated.tsv", 
                       header = T) %>% dplyr::select(gene_name) %>% unlist() 
ENTREZ_universe <- unique(unlist(strsplit(ENTREZ_universe, ",")))
ENTREZ_universe <- bitr(ENTREZ_universe, fromType = "SYMBOL", 
